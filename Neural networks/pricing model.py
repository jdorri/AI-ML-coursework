################################################################################
# Author: Faidon, Maleakhi, Harry, Jamie
# Date: 18/November/2019
# Description: Pricing model which is used for insurance pricing. This model will
# be used in a competitive AI market.
################################################################################
import numpy as np
import pandas as pd ## Used for main
import pickle
...
class PricingModel():
    ...
    def __init__(self, calibrate_probabilities=False, params=None):
        """
        Constructor for the pricing model class.

        Parameters
            params {dict} -- dictionary representing the best parameters (see part 2 for detailed explanation)
        """
        self.y_median = None
        ...
        if not params:
            # For dummy instances can get the access _preprocessor
            self.base_classifier = None
        else:
            # Initialise keras base_classifier
            self.base_classifier = KerasClassifier(self.create_model, **params)
        ...
        # Variables to set standard columns for Neural Network inputs
    ...
    def fit(self, X_raw, y_raw, claims_raw):
        ...
        self.base_classifier.fit(X_train_smoted, y_train_smoted, epochs=5, batch_size=32, validation_split=0.1)
    ...
    def predict_claim_probability(self, X_raw):
        ...
        return y_predict # return probabilities for the positive class (label 1)
    ...
    def predict_premium(self, X_raw):
        ...
        """
        # Add premium of 1%
        return self.predict_claim_probability(X_raw) * self.y_median * 1.01
    ...
    def create_model(self, hidden_layers, neurons, input_dim):
        ...
        # Setup model
        # By default, set 100 neurons each layer, relu activation function
    ...
    def upsample(self, X_train, y_train):
        ...
        for i in range(int(num_updates/4)):
            ...
            # Here I am removing the the double square brackets
            nearest_neigh = nearest_neigh[0]
            nearest_neigh = nearest_neigh[0]
            # Now the returned index needs to be converted back to the pandas index
        ...
        df_new_data_y = pd.DataFrame(new_data_y)#, columns = list(y_train.columns))
...
if __name__ == "__main__":
    ...
    """
    # The parameters that can be obtained using grid search.
    # For grid search, use code similar to part 2. We run that by importing it.
    param_grid = {
        "epochs": 5,
        "input_dim": 43,
        "hidden_layers": 2,
    ...
    pricing_model = PricingModel(params=param_grid)
    ...
    # Predict probability and premium
